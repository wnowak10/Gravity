...
One idea for a helpful numerical descriptor could be s, where s = b1 + b2, where b1 is the fraction of output 1 in the first category (call this g<sub>1</sub>) at split a, and b2 is the fraction of output 1 in the second category (call this g<sub>2</sub>).

In this case, splitting on feature "Temperature" at the median, we find that s<sub>a1</sub> = 1 + .6 = 1.6. It seems that an ideal split would result in two pure splits, leaving us with s = 1 + 1 = 2. On the contrary, a non-informative split would do no better than chance. In binary classification, this should result in 50% success along each branch. So, we have 1 < s < 2.    
