...
One idea for a helpful numerical descriptor could be s, where s = b1 + b2, where b1 = max(x, 1-x) with x = sum(elements in branch 1)/(c<sub>1</sub>), where c<sub>1</sub> is the number of elements in branch 1 (c for cardinality). b2 is defined similarly for the second branch. 
...
A good attribute splits data evenly. In example 1 below, the attributes acheive an s score of 1 + .89 = 1.89. However, it doesn't split the data very evenly.
