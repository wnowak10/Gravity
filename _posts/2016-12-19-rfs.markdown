...
One idea for a helpful numerical descriptor could be s, where s = b1 + b2, where b1 = max(x, 1-x) with x = sum(elements in branch 1) / (number of elements in branch 1). b2 is defined similarly for the second branch. 
...
In addition to judging our split attribute on its ability to successfully classify, we can generate another metric to judge various split attribues.  

A good attribute splits a lot of data. In example below, both attributes acheive an s score of 1.6. However, the attribute in example 1 is, in some sense, a worse attribute than that in example 2, as it bins less of the total data. It does less work, or is less productive, if you will. So we could say that another metric of interest is avg, where avg = average # elements in branch 1 and branch 2.

In the example above, avg1 was 1+5 / 2 = 3.
