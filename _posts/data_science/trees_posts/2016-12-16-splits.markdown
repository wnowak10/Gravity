...
The situation is a bit simpler for numerical dependent variables. Unfortunately, I cheated a bit here, and looked some at Hastie's [ESL](http://statweb.stanford.edu/~tibs/ElemStatLearn/). On page 307, the show that the problem is optimizable (is that a word?). We just need to find variable j and split point s that leads to a minimum squared error term. 
